...
import { mockSources } from "@/src/data/mockData";
...
export function SourceManager() {
  const [sources, setSources] = useState<SourceDefinition[]>(mockSources);
  return (
    <div className="space-y-5">
      <header className="rounded-3xl border border-slate-200 bg-white p-6 shadow-soft">
        ...
        <p className="mt-2 text-slate-600">
          Local toggles show how source quality, bias risk, and confirmation
          requirements shape trust.
        </p>
      </header>
      {groups.map((group) => (
        <section
